...
const SHORT_STRING_SIMILARITY = 0.8
const MEDIUM_STRING_SIMILARITY = 0.7
const LONG_STRING_SIMILARITY = 0.55
...
function getTagsFromCategories(categories) {
	...
	const matchingTags = tags.filter(t => {
		return categoryTitles.some(c => advancedMatchCheck(t, c))
	})
...
function getDirNameFromCategory(category) {
	...
	return encodeURI(directoryName)
}

async function getAdditionalCategories(categories) {
	const { CategoryData } = categories
	const directoriesToBrowse =
		CategoryData.constructor === Array
			? CategoryData.map(getDirNameFromCategory)
			: [getDirNameFromCategory(CategoryData)]
	...
	const additionalCategories = responseDataParsed.map(d => {
		const categoryObj =
			d.Awis.Results.Result.Alexa.CategoryBrowse.Categories.Category
		const categories =
			categoryObj.constructor === Array
				? categoryObj.map(c => c.Title)
				: categoryObj.Title
...
// Only removes duplicates currently but can be useful to blacklist tags in the future
...
async function categorizeAdSlot(adSlot) {
	...
	if (!initialCategories) {
		// if website isn't categorized by Alexa further checks are not needed and will only cause errors
		return []
	}
	try {
		...
		console.error('Error retrieving additional categories and tags', err)
		return []
	}
}
