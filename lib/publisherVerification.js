...
async function getRank(websiteUrl) {
	if (!AWIS_KEY) return null
	const { hostname } = url.parse(websiteUrl)
	const resp = await fetch(
		`https://awis.api.alexa.com/api?Action=UrlInfo&ResponseGroup=Rank&Url=${hostname}`,
		{
	...
	//if (!Awis.Results[0]) return { noResult }
	//console.log(Awis.Results[0].Result[0].Alexa[0].TrafficData[0].UsageStatistics[0])
	const rankStr = Awis.Results[0].Result[0].Alexa[0].TrafficData[0].Rank[0]
	return rankStr ? parseInt(rankStr, 10) : 0
}
...
module.exports = async function verifyPublisher(addr, websiteUrl, opts = {}) {
	...
	const [verifiedIntegration, verifiedOwnership, rank] = await Promise.all([
