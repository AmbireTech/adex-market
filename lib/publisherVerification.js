...
async function verifyOwnership(publisherAddr, websiteUrl) {
	...
	if (response.status !== 200) throw new Error('website does not return HTTP 200')
	...
	return html.includes(`publisherAddr%22%3A%22${publisherAddr}`) && html.includes(`src="https://viewm.moonicorn.network`)
}
