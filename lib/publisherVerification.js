...
const isPlaystationPublisher = id =>
	PLAYSTATION.some(p => p === id)
...
async function getAlexaStats(websiteUrl) {
	...
	// TODO: remove alexa stuff from other places in the platform and supermarket 
	return {
		alexaDataUrl: websiteUrl,
		rank: 1, // Because of the external fraud detection
		reachPerMillion: 0
	...
	// 	}
	// )
	// if (resp.status !== 200) throw new Error(`alexa returned ${resp.status}`)
...
async function verifyPublisher(addr, websiteUrl) {
	...
	const ifBlacklisted = (isHostnameBlacklisted(hostname) || isPlaystationPublisher(publisher))
		? { blacklisted: true }
		: {}
	return {
		publisher,
