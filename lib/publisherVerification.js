...
async function getWebshrinkerCategories(website) {
	...
	const Authorization = 'Basic ' + Buffer.from(WEBSHRINKER_KEY_AND_SECRET).toString('base64')
	const rootName = hostname.split('.').slice(-2).join('.')
	const resp = await fetch(
		`https://api.webshrinker.com/categories/v3/${Buffer.from('https://'+hostname).toString('base64')}`,
		{
	...
	const webshrinkerIABCategories = categories
		...
		.map(x => {
			if (!x.id.startsWith('IAB')) throw new Error(`invalid category ID: ${x.id}`)
			return x.id
		})
...
async function verifyOwnershipWellKnown(publisherAddr, websiteUrl) {
	const { hostname } = url.parse(websiteUrl)
	try {
		const resp = await fetch(`https://${hostname}/.well-known/adex.txt`, { timeout: 3000 })
...
async function detectExtraFlags(publisherAddr, websiteUrl) {
	...
	const INCENTIVIZED_REGEX = /earn (bitcoin|money|free)|paid to click|ptc ads|faucet( )?pay|paid faucet|(bitcoin,dogecoin,litecoin) faucet/ig
	try {
		const resp = await fetch(websiteUrl, { timeout: 5000, redirect: 'follow' })
		if (resp.status !== 200) return {}
		const text = await resp.text()
		if (text.match(INCENTIVIZED_REGEX)) return { 'webshrinkerCategoriesOverrides.incentivized': true }
		else return {}
	} catch (e) {
		return {}
	}
