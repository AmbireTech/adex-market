...
async function verifyOwnership(publisherAddr, websiteUrl) {
	...
	return dnsResp.Answer.some(
		record =>
			(record.name === `${hostname}.` || record.name === hostname || record.name === rootName) &&
			(record.data === `"adex-publisher=${publisherAddr}"` || record.data === `adex-publisher=${publisherAddr}` || record.data === publisherAddr)
	)
}
...
async function getAlexaStats(websiteUrl) {
	...
	// First one in UsageStatistics is for the last 3 months
	const usageStats = trafficData.UsageStatistics ? trafficData.UsageStatistics[0].UsageStatistic : null
	const reachPerMillion = usageStats ? usageStats.Reach[0].PerMillion[0].Value[0] : null
	const rankStr = Awis.Results[0].Result[0].Alexa[0].TrafficData[0].Rank[0]
	const rank = rankStr ? parseInt(rankStr, 10) : 0
	return { rank, reachPerMillion }
}
...
module.exports = async function verifyPublisher(addr, websiteUrl, opts = {}) {
	...
	const [verifiedIntegration, verifiedOwnership, alexaStats] = await Promise.all([
