...
const SINGLE_ELEMENT_TLDS = require('./singleElementTlds')
...
function getRootName(hostname) {
	...
	if (split_hostname[0] === 'www') {
		split_hostname.splice(0, 1)
	}
	if (split_hostname.length > 2) {
		let index_to_split = SINGLE_ELEMENT_TLDS.includes(
			split_hostname[split_hostname.length - 1]
		)
			? -2
			: -3

		return split_hostname.slice(index_to_split).join('.')
	}
