...
function isDisconnected (messagesFromAll) {
	const { followerHeartbeat, followerHbFromLeader } = messagesFromAll

	const recentLeaderHbFromFollower = getRecentHeartbeats(followerHeartbeat)
	const recentFollowerHbFromLeader = getRecentHeartbeats(followerHbFromLeader)

	return !(recentLeaderHbFromFollower.length && recentFollowerHbFromLeader.length)
...
function isActive (messagesFromAll) {
	const { leaderHeartbeat, followerHeartbeat, newStateLeader, approveStateFollower } = messagesFromAll
	const isHealthy = approveStateFollower[0] ? approveStateFollower[0].msg.isHealthy : false
...
function isWithdraw (campaign) {
	return campaign.spec.withdrawPeriodStart
