...
// there are recent NewState messages, but the follower issues RejectState instead of ApproveState
function isRejected(messagesFromAll) {
	const { newStateLeader, rejectStateFollower } = messagesFromAll
	if (newStateLeader[0] && rejectStateFollower[0]) {
		return true
	}
	return false
}
