...
function isInitializing (messagesFromBothValidators) {
	return messagesFromBothValidators.some((m) => {
		return m.length === 0
	})
}

// at least one validator doesn't have a recent Heartbeat message
function isOffline (messagesFromBothValidators) {
	return messagesFromBothValidators.some((m) => {
		console.log(m.filter(filterMessages('Heartbeat', true)))
