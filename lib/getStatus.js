...
function isInvalid(messagesFromAll) {
	...
	console.log(newStateLeader[0])
	// If there is a RejectState, there won't be a recent NewState/ApproveState
	if (
		!isDateRecent(newStateLeader[0].received) ||
		!isDateRecent(approveStateFollower[0].received)
	) {
		return true
	}
	return false
