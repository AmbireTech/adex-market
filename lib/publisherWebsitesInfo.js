...
async function getWebsitesInfo(websitesCol, slot) {
	if (slot.website) {
		...
		const categories = websitesWithNoDupes
			.map(getCategories)
			.reduce((a, b) => a.concat(b), [])
			.filter((x, i, cats) => cats.indexOf(x) === i)
		return {
			acceptedReferrers,
			alexaRank:
				websitesWithNoDupes.length === 1 ? websitesWithNoDupes[0].rank : null,
			categories,
		}
	}
}

module.exports = { getWebsitesInfo, getCategories }
