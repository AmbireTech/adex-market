...
tape('GET /campaigns', t => {
	fetch(`${marketUrl}/campaigns`)
		...
		.then(res => {
			...
			t.equals(
				res.length,
				constants.ACTIVE_CAMPAIGNS,
				'returns right amount of campaigns'
			)
			t.ok(res[0].hasOwnProperty('id'), 'campaign has property id')
			t.ok(res[0].hasOwnProperty('status'), 'campaign has property status')
			t.ok(
				res[0].hasOwnProperty('depositAsset'),
				'campaign has property depositAsset'
			)
			t.ok(
				res[0].hasOwnProperty('depositAmount'),
				'campaign has property depositAmount'
			)
			t.ok(res[0].hasOwnProperty('spec'), 'campaign has property spec')
			t.ok(
				res[0].spec.hasOwnProperty('validators'),
				'campaign has property validators'
			)
			t.ok(
			...
			t.ok(
				res[0].status.hasOwnProperty('name'),
				'campaign status has name property'
			)
			t.equals(
				typeof res[0].depositAsset,
				'string',
				'property depositAsset is of type string'
			)
			t.equals(
				typeof res[0].depositAmount,
				'string',
				'property depositAmount is of type string'
			)
			t.ok(
				Array.isArray(res[0].spec.validators),
				'campaign validators is an array'
			)
	...
	fetch(`${marketUrl}/campaigns/${testData.campaigns[0].id}`)
		...
		.then(res => {
			t.equals(
				...
				'[object Object]',
				'returns array with the element'
			)
...
tape('GET /users/list', t => {
	fetch(`${marketUrl}/users/list`)
		...
		.then(res => {
			...
			t.equals(
				res[0].signerAddress,
				'0x2aecf52abe359820c48986046959b4136afdfbe2',
				'correct address'
			)
			t.end()
		})
	...
	fetch(`${marketUrl}/stats`)
		...
		.then(res => {
			...
			t.ok(
				...
				'stats has anonAdvertiserCount'
			)
			t.ok(
				res.hasOwnProperty('campaignCount'),
				'stats has campaignCount property'
			)
			t.ok(
				res.hasOwnProperty('campaignsByStatus'),
				'stats has property campaignsByStatus'
			)
			t.ok(
				res.hasOwnProperty('totalSpentFundsByAssetType'),
				'stats has totalSpentFundsByAssetType'
			)
			t.equals(
				typeof res.publisherCount,
				'number',
				'publisherCount is of correct type'
			)
			t.equals(
				typeof res.advertiserCount,
				'number',
				'advertiserCount is of correct type'
			)
			t.equals(
				typeof res.anonPublisherCount,
				'number',
				'anonPublisherCount is of right type'
			)
			t.equals(
				typeof res.anonAdvertiserCount,
				'number',
				'anonAdvertiserCount is the right type'
			)
			t.equals(
				typeof res.campaignCount,
				'number',
				'campaignCount is of right type'
			)
			t.equals(
				typeof res.campaignsByStatus,
				'object',
				'campaignsByStatus is of right type'
			)
			t.equals(
				typeof res.totalSpentFundsByAssetType,
				'object',
				'totalSpentFundsByAssetType is of right type'
			)
			...
			t.equals(
				res.anonAdvertiserCount,
				0,
				'anonAdvertiserCount is of right amount'
			)
			t.equals(
				res.campaignCount,
				testData.campaigns.length,
				'campaignCount is of right amount'
			)
			t.equals(
				res.campaignsByStatus['Active'],
				testData.campaigns.filter(c => c.status.name === 'Active').length,
				'active status campaigns are the right amount'
			)
			t.equals(
			...
			fetch(`${marketUrl}/users/list`)
				...
				.then(res => {
					...
					t.equals(
						res.length,
						2,
						'2 users, the previous one + the recently added one'
					)
					t.ok(
						res.some(
							obj =>
								obj.identity === testData.user.identity &&
								obj.signature === testData.user.signature
						),
						'the new object has been added'
					)
					t.ok(
						addrRegex40.test(res[0].signerAddress) &&
							addrRegex40.test(res[1].signerAddress),
						'Adress of users is real address'
					)
					t.ok(
						addrRegex40.test(res[0].identity) &&
							addrRegex40.test(res[1].identity),
						'Identity of users is real address'
					)
					t.ok(
						addrRegex130.test(res[0].signature) &&
							addrRegex130.test(res[1].signature),
						'Signature of users is real signature'
					)
					t.ok(
						addrRegex64.test(res[0].hash) && addrRegex64.test(res[1].hash),
						'Hash of users is correct'
					)
					t.end()
				})
		})
...
tape('===== Authorized routes =====', t => {
	fetch(`${marketUrl}/auth`, {
		...
		.then(authResult => {
			...
			form.append(
				'media',
				fs.createReadStream(`${__dirname}/../resources/img.jpg`)
			)
			...
			const getAdUnits = fetch(`${marketUrl}/units`)
				...
				.then(res => {
					...
					t.ok(
						res[0].hasOwnProperty('mediaUrl'),
						'adUnit has property mediaUrl'
					)
					t.ok(
						res[0].hasOwnProperty('targetUrl'),
						'adUnit has property targetUrl'
					)
					t.ok(
						res[0].hasOwnProperty('mediaMime'),
						'adUnit has property mediaMime'
					)
...
tape('GET /session unauthenticated', t => {
	fetch(`${marketUrl}/session`).then(res => {
...
tape('GET /tags', t => {
	fetch(`${marketUrl}/tags`)
		...
		.then(res => {
			...
			t.ok(
				res.every(i => i.hasOwnProperty('_id')),
				'All tags have IDs'
			)
			t.ok(
				res.every(i => typeof i['_id'] === 'string'),
...
tape('GET /campaigns?byEarner&limitForPublisher... FILTERING', t => {
	fetch(
		...
		.then(res => {
			...
			t.equals(
				res.length,
				cfg.maxChannelsEarningFrom, // There are 10 more channels than the limit so it should limit them
...
tape('GET /campaigns?byEarner', t => {
	fetch(
		`${marketUrl}/campaigns?status=Active,Ready&byEarner=${byEarnerIdentity}`
	)
		.then(res => res.json())
		.then(res => {
			t.ok(Array.isArray(res), 'returns array')
			t.equals(
				res.length,
				37, // basically all test channels with this identity address, since we dont limit
				'right amount of campaigns are returned'
			)
			t.ok(
				res.every(c => c.status.name === 'Active' || c.status.name === 'Ready'),
			...
			t.ok(
				res.every(c =>
					c.status.lastApprovedBalances.hasOwnProperty(byEarnerIdentity)
				),
				'Each campaign contains byEarnerIdentity in balances'
			)
			t.end()
		})
})
