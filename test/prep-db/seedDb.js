...
function getCampaign (options) {
	return {
		'_id': options.id || '0xceb6ab03139b98e0a22d4375ce658759b8729e21c783dea9d385d99f76527865',
		...
		'id': options.id || '0xceb6ab03139b98e0a22d4375ce658759b8729e21c783dea9d385d99f76527865',
		'spec': {
			...
			'maxPerImpression': (options.spec && options.spec.maxPerImpression) || '1000000000000000',
			'minPerImpression': (options.spec && options.spec.minPerImpression) || '1000000000000000',
			'targeting': (options.spec && options.spec.targeting) || [ ],
			'created': (options.spec && options.spec.created) || 1558351283397,
			'nonce': (options.spec && options.spec.nonce) || '73925552938438451124360388946703653167557904034069293949858925105243906956203',
			'withdrawPeriodStart': (options.spec && options.spec.withdrawPeriodStart) || 1558437660000,
			'eventSubmission': {
				'allow': [
					{
						'uids': [
		...
		'status': {
			...
			'lastApprovedBalances': (options.status && options.status.lastApprovedBalances) || {
				'0x712e40a78735af344f6ae3b79fa6952d698c3b37': '1000000000000000000'
			},
			'verified': (options.status && options.status.verified) || true,
			'lastChecked': (options.status && options.status.lastChecked) || 1562143408103,
			'fundsDistributedRatio': (options.status && options.fundsDistributedRatio) || 1000,
