...
function verifyLastApproved(lastApproved, validators) {
	...
	const approveStateAddr = verifyMessage(
		approveStateMsg,
		approveState.msg.signature
	)

	if (
		newStateAddr === newState.from &&
		approveStateAddr === approveState.from
	) {
...
function doesMsgMatchValidators(newStateAddr, approveStateAddr, validators) {
	return (
		newStateAddr === validators[0].id && approveStateAddr === validators[1].id
	)
}
